...
The penguins dataset contains some penguin information. Important variables include species(Adelie, Chinstrap, and Gentoo), island, bill length (mm), bill depth (mm), flipper length (mm), body mass (g), sex and year. The size of dataset has `r nrow(penguins)` rows and `r ncol(penguins)` columns. 
...
This explain why only numeric and logical variables can have a mean.